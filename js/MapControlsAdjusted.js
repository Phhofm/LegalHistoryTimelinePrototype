...
/** ADAPTED FOR LEGALHISTORYTIMELINE, see keyword 'adapted' in commments to see changes
 * @author phhofm
 */
...
var MapControls = function ( object, domElement ) {
    ...
    // adapted for legaltimeline
    // only up&down panning in walkView
    this.disableHorizontalPanning = false;
    this.disableVerticalPanning = false;
    this.isometricControls = false; // converts X-movement to Z-movement panning

    this.limitPanningZNegative= Infinity;
    this.limitPanningZPositive= Infinity;
    ...
    var panLeft = function () {
        ...
        return function panLeft(distance, objectMatrix) {
            ...
            if (scope.disableHorizontalPanning) {} //disable
            else if (scope.isometricControls){ //isometricView: if we drag sideways we move on the z axis
                //TODO implement restriction here. This is hardcoded find appropriate values from timeline and timeevents
                if(distance < -0.1 && scope.object.position.z + distance < scope.limitPanningZNegative){}else if (distance > 0.1 && scope.object.position.z + distance > scope.limitPanningZPositive){} else { //limits panning capabilities for DAU case preventing user to loose view

                var p = new Vector3(0,0,distance);
                panOffset.add(p);}
            } else {
...
// adapted for legaltimeline
    var panUp = function () {
        ...
        return function panUp(distance, objectMatrix) {
            if (scope.disableVerticalPanning) {} else { // deactivate
